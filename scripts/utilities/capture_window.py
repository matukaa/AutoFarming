...
def _get_required_window_size_for_client(hwnd: int, client_width: int, client_height: int, style: int, ex_style: int) -> tuple[int, int]:
    """Compute required window size for a desired client area, DPI-aware when available."""
...
def resize_7ds_window(width=540, height=960):
    ...
    try:
        ...
        # Final check + correction loop (handles DWM/invisible frame differences)
        for attempt in range(3):
            ...
            print(
                f"[DEBUG] Correction pass {attempt + 1}: adjusting by {width_diff}x{height_diff}"
            )
